...
import { deleteUser, getProfile, updateProfile } from "@/services/auth";
...
export default function EditPage() {
  ...
  useEffect(() => {
    if (isSuccess) {
      setCompanyName(data.distributor.company_name);
      setName(data.distributor.name);
      setBin(data.distributor.bin);
      setPhone(data.distributor.phone_number);
      setCity(data.distributor.city);
      setDetails(data.distributor.details);
      setImgURL(data.distributor.img_url);
    }
  }, [isSuccess]);
        ...
        <div className="mt-[20px] flex justify-between w-full mb-[70px]">
          <div className="flex flex-col">
            <button
              ...
              onClick={() => router.push("/distributor/profile/edit/password")}
            >
              Change password
            </button>
            <button
